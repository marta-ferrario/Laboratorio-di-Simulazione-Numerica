...
double Random :: Exp(double lambda){
   // This function generates a random number from a generic exponential distribution p(x)= lambda e^{-x lambda} with given lambda
   double r=Rannyu();
   double x= - (1/lambda)*log(1-r);
   return x; 
}

double Random :: CL(double mu, double gamma){
   // This function generates a random number from a generic exponential distribution p(x)= lambda e^{-x lambda} with given lambda
   double r=Rannyu();
