...
void System :: move(int i){ // Propose a MC move for particle i
  ...
  if(_sim_type == 3){ //Gibbs sampler for Ising
    ...
    // calcolo del campo efficace
    double heff = _J * (_particle(this->pbc(i-1)).getspin() + _particle(this->pbc(i+1)).getspin()) + _H; 

    double p = 1/(1+ exp(-2*_beta*heff)); 
    double val = _rnd.Rannyu();
    if (val < p){
...
void System :: measure(){ // Measure properties
  ...
  if(_measure_pofv){
    ...
    for(int i=0; i<_npart; i++){
      ...
      bin=int(v/_bin_size_v);  // indice del bin in cui mettere la velocità
      if(bin < _n_bins_v and bin>=0){
  ...
  if (_measure_magnet) {
    magnetization=0.; 
    for (int i=0; i < _npart; i++){
      magnetization += _particle(i).getspin(); // magnetizzazione totale del sistema 
    }
    magnetization=magnetization;
    _measurement(_index_magnet)=magnetization/_npart; // magnetizzazione per spin
  }
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
  if (_measure_cv){
    double energy = tenergy_temp; 
    _measurement(_index_cv) = energy * energy; // salvo solo l'energia al quadrato
  }
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  if (_measure_chi){
...
void System :: averages(int blk){
  ...
  if (_measure_tenergy){
    ...
    if(blk == _nblocks and _sim_type == 3 and _H == 0){
      coutf.open("../OUTPUT/tenergy_gibbs.dat",ios::app);
      coutf << setw(12) << _temp
          << setw(12) << average
          << setw(12) << sum_average/double(blk)
          << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
    ...
    if(blk == _nblocks and _sim_type == 2 and _H == 0){
      coutf.open("../OUTPUT/tenergy_metro.dat",ios::app);
      coutf << setw(12) << _temp
          << setw(12) << average
          << setw(12) << sum_average/double(blk)
          << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
  ...
  if (_measure_pofv){
    ...
    double sum = 0.0;  // per controllo normalizzazione

    for(int k = 0; k < _n_bins_v; k++){

      int idx = _index_pofv + k;
      average = _average(idx);
      sum_average = _global_av(idx);
      sum_ave2 = _global_av2(idx);

      double v = (k + 0.5) * _bin_size_v;   // centro del bin
      double norm = _npart * _bin_size_v;
    
      double value = average / norm;
      double global = 1.* (sum_average) / (norm*blk); 
      double err = this->error(sum_average, sum_ave2, blk)/ norm;
      sum += global * _bin_size_v;
    ...
    // // stampa SOLO alla fine della simulazione
    // if (blk == _nblocks){
    //   cout << "Integrale distribuzione = " << sum << endl;
    // }
  }
  // //
  ...
  if (_measure_magnet){
    ...
    if(blk == _nblocks and _sim_type == 3 and _H == 0.02){ // stampa all'ultimo blocco
      coutf.open("../OUTPUT/magnet_gibbs.dat",ios::app);
      coutf << setw(12) << _temp
          << setw(12) << average
          << setw(12) << sum_average/double(blk)
          << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
    ...
    if(blk == _nblocks and _sim_type == 2 and _H == 0.02){
      coutf.open("../OUTPUT/magnet_metro.dat",ios::app);
      coutf << setw(12) << _temp
          << setw(12) << average
          << setw(12) << sum_average/double(blk)
          << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
  ...
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
  if (_measure_cv){
    double av_e = _average(_index_tenergy); 
    double av_e2 = _average(_index_cv);
    
    double cv_block = _beta*_beta * (av_e2 - av_e*av_e) * _npart;

    _global_av(_index_cv) += cv_block -av_e2; 
    _global_av2(_index_cv) += cv_block*cv_block - av_e2*av_e2;
    ...
    if(blk == _nblocks and _sim_type == 3 and _H == 0){
      coutf.open("../OUTPUT/cv_gibbs.dat",ios::app);
      coutf << setw(12) << _temp
          << setw(12) << average
          << setw(12) << sum_average/double(blk)
          << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
    ...
    if(blk == _nblocks and _sim_type == 2 and _H == 0){
      coutf.open("../OUTPUT/cv_metro.dat",ios::app);
      coutf << setw(12) << _temp
          << setw(12) << average
          << setw(12) << sum_average/double(blk)
          << setw(12) << this->error(sum_average, sum_ave2, blk) << endl;
  ...
  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
    if(_measure_chi){
      ...
      double chi_blk = _beta*average/_npart; 
      double chi_mean = _beta*sum_average/double(blk)/_npart; 
      double chi_err = _beta * this->error(sum_average, sum_ave2, blk)/_npart; 

      coutf << setw(12) << blk
          << setw(12) << chi_blk
          << setw(12) << chi_mean
          << setw(12) <<  chi_err << endl;
      coutf.close();


      if(blk == _nblocks and _sim_type == 3 and _H == 0){
        coutf.open("../OUTPUT/chi_gibbs.dat",ios::app);
        coutf << setw(12) << _temp
            << setw(12) << chi_blk
            << setw(12) << chi_mean
            << setw(12) << chi_err << endl;
      ...
      if(blk == _nblocks and _sim_type == 2 and _H == 0){
        coutf.open("../OUTPUT/chi_metro.dat",ios::app);
        coutf << setw(12) << _temp
            << setw(12) << chi_blk
            << setw(12) << chi_mean
            << setw(12) << chi_err << endl;
...
void System::equilibrate(int nstep){
  for(int i=0; i<nstep; i++) step();
}