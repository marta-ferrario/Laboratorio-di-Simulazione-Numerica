...
void System :: initialize_velocities(){
  ...
  if(_restart){
    ifstream cinf;
    cinf.open("../INPUT/CONFIG_restart_MD/conf-1.xyz");
    if(cinf.is_open()){
      string comment;
      string particle;
      int ncoord;
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        // ESERCIZIO 7.1
        _vtail = ((8.0*M_PI*_rho)/3.0) * ((1.0/(3.0*pow(_r_cut,9)))-1/pow(_r_cut,3)); 
        // cout << _vtail << endl; 
        // //
        ...
        index_property++;
        // ESERCIZIO 7.1
        _ptail = ((32.0*M_PI*_rho)/3.0) * ((1.0/(3.0*pow(_r_cut,9)))-1/(2*pow(_r_cut,3))); 
        // cout << _ptail << endl; 
        // //
...
void System :: read_configuration(){
  ifstream cinf;
  cinf.open("../INPUT/CONFIG_restart_MD/config.xyz");
  if(cinf.is_open()){
    string comment;
    string particle;
    double x, y, z;
...
void System :: averages(int blk){
  ...
  if (_measure_pofv){
    ...
    for(int k = 0; k < _n_bins; k++){
      ...
      double gr = (k + 0.5) * _bin_size;   // centro del bin
      double deltaV = (4*M_PI/3)*(pow(k*_bin_size+_bin_size,3)-pow(k*_bin_size,3));
      double norm = _npart * _bin_size * _rho * deltaV;
...
void System::write_observable(int step){
    ofstream coutf;
    coutf.open("../OUTPUT/instant_penergy.dat", ios::app);
