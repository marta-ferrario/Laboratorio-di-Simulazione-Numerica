...
void System :: initialize(){ // Initialize the System object according to the content of the input files in the ../INPUT/ directory
  ...
  // ESERCIZIO 4.2: OCCUPAZIONE DI META' FCC
  for(int i=0; i< _npart; i++){
    double x = _particle(i).getposition(0,true); 
    double y = _particle(i).getposition(1,true); 
    double z = _particle(i).getposition(2,true); 

    _particle(i).setposition(0, x*0.5); 
    _particle(i).setposition(1, y*0.5); 
    _particle(i).setposition(2, z*0.5); 
...
void System :: initialize_velocities(){
  ...
  // ESERCIZIO 4.3: INVERSIONE VELOCITA'
  if(_restart){
    ifstream cinf;
    ifstream cinf_old;
    cinf_old.open("../INPUT/CONFIG/conf-1_short.xyz");
    cinf.open("../INPUT/CONFIG/config_short.xyz");
    ...
    if(cinf.is_open()){
      if(cinf_old.is_open()){
        ...
        for(int i=0; i<_npart; i++){
          cinf >> particle >> x >> y >> z;
          cinf_old >> particle >> xold >> yold >> zold;
          ...
          _particle(i).setposition(0, pbc(_side(0)*xold,0));
          _particle(i).setposition(1, pbc(_side(1)*yold,1));
          _particle(i).setposition(2, pbc(_side(2)*zold,2));

        }
      }
    } else cerr << "PROBLEM: Unable to open INPUT files"<< endl;
    cinf.close();
    cinf_old.close();
    // //
    ...
    // ESERCIZIO 4.2: INIZIALIZZAZIONE VELOCITA'
...
void System :: averages(int blk){
  ...
  //   double N_grafici=5; 
  //   int passo_grafici= int(_nblocks/N_grafici);
  //   int file_index = blk/passo_grafici;

    
  //   vector<string> nomi_file;
  //   for (int i=0; i<=N_grafici+1; i++){
  //     nomi_file.push_back("../OUTPUT/pofv_"+ to_string(i)+ ".dat"); 
  //   }

  //     if((blk % passo_grafici) == 0 or blk == 1){
  //       if(blk == 1){
  //         file_index = 0; 
  //       }
  //       coutf.open(nomi_file[file_index]);

  //       // cout << "Blocco: " << blk << endl; 
  //       // cout << "File_index: " << file_index << endl << endl; 
